...
const Category = () => {
  ...
  const handleLike = (product: CategoryType, event: React.MouseEvent<HTMLDivElement, MouseEvent>) => {
    ...
    const isLiked = likedProducts.some((p: CategoryType) => p.id === product.id);
    if (isLiked) {
  ...
  const handleCart = (product: CategoryType, event: React.MouseEvent<HTMLDivElement, MouseEvent>) => {
    ...
    const isCarted = cartedProducts.some((p: CategoryType) => p.id === product.id);
    if (isCarted) {
      ...
      <div className="Category__wrapper">
        ...
        <div className="Category__products">
          {products.map((product) => {
            const isLiked = likedProducts.some((p: CategoryType) => p.id === product.id);
            const isCarted = cartedProducts.some((p: CategoryType) => p.id === product.id);// Check if the product is liked
            return (
